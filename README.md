...
**Windows 桌面 API 中转管理器** — 把任何 OpenAI-Compatible 模型（GLM、DeepSeek、通义千问、Moonshot…）转换成 OpenAI Responses API，给 Codex / ccswitch 等工具用。带 GUI、多账号、一键切换。
...
- 配置持久化在 `%APPDATA%\GLMBridge\config.json`
...
```cmd
git clone https://github.com/ninxmaker/bridge-app.git
cd bridge-app
dev.bat
```

首次运行自动安装依赖（约 1 分钟），之后直接启动 GUI。
...
产物在 `dist\GLMBridge.exe`，单文件约 30-50MB，双击运行不依赖 Python。

## 客户端配置

在 Codex / ccswitch / Cockpit Tools 等工具里加一个 channel：
...
多个中转站（GPT 中转、GLM、DeepSeek…）可以同时在 ccswitch 里配不同 channel，按需切换。

## 架构

```
┌──────────────┐      ┌─────────────────────────────┐      ┌──────────────┐
│  Codex /     │      │       GLM Bridge            │      │  上游模型     │
│  ccswitch    │─────▶│  http://127.0.0.1:4000/v1   │─────▶│  GLM / DS /  │
...
├── config_store.py     # 配置存储 (%APPDATA%\GLMBridge)
├── providers.py        # 内置 Provider 预设
├── ui/
│   └── index.html      # 前端 UI（HTML + 内联 CSS/JS）
├── requirements.txt
├── GLMBridge.spec      # PyInstaller 配置
├── build.bat           # 一键打包成 exe
├── dev.bat             # 开发模式直接运行
└── README.md
```
...
- **桌面**：PyWebView（EdgeWebView2 内核，Win11 自带）
